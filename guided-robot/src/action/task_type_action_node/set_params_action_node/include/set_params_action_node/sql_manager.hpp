...
public:
     ...
     * 通过列名查找数据行，并更新指定列的值
     * @param filename  文件路径
     * @param searchCol 目标列名称（用于查找）
     * @param searchVal 要查找的值
     * @param updateCol 要更新的列名称
     * @param updateVal 新的值
     * @return 更新成功的行数
     *
     * SqlManagerNode sqlManager;
int updatedRows = sqlManager.UpdateRowByColumn("data.csv", "Name", "John", "Age", "30");
std::cout << "Updated " << updatedRows << " rows." << std::endl;
     */
    int UpdateRowByColumn(
        const std::string &filename,
        const std::string &searchCol,
        const std::string &searchVal,
        const std::string &updateCol,
        const std::string &updateVal)
    {
        ...
        {
            headers.push_back(headerCell);
            if (headerCell == searchCol)
            {
                searchColIndex = headers.size() - 1;
            }
            if (headerCell == updateCol)
            {
                updateColIndex = headers.size() - 1;
            }
        }

        if (searchColIndex == -1)
        {
            std::cerr << "Search column not found: " << searchCol << std::endl;
            return 0;
        }

        if (updateColIndex == -1)
        {
            std::cerr << "Update column not found: " << updateCol << std::endl;
            return 0;
        }
        ...
        while (std::getline(file, dataLine))
        {
            ...
            if (searchColIndex < row.size() && row[searchColIndex] == searchVal)
            {
                if (updateColIndex < row.size())
                {
                    row[updateColIndex] = updateVal;
                }
            }
        ...
        // 写回CSV文件‌:ml-citation{ref="1,3" data="citationList"}
        WriteCSVWithHeaders(filename, headers, data, false);
