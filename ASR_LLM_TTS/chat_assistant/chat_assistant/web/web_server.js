...
const els = {
  ...
  liveViewerTitle: document.getElementById("liveViewerTitle"),
...
function switchTab(tabName) {
  ...
  if (showLogs) {
    if (state.liveLogName) startPollingLiveLog(state.liveLogName, false);
  } else {
    stopPolling();
...
function renderLogList() {
  ...
  for (const log of state.logs) {
    ...
    tag.className = "tag";
    tag.textContent = log.is_live ? "实时" : "历史";
    if (!log.is_live) {
...
async function selectLog(logName) {
  const log = state.logs.find(item => item.name === logName);
  if (log?.is_live) {
    state.selectedLog = logName;
    renderLogList();
    els.liveViewer.classList.remove("hidden");
    els.historyViewer.classList.add("hidden");
    els.liveViewerTitle.textContent = `实时日志 (${logName})`;
    startPollingLiveLog(logName, true);
    return;
  }
...
function startPollingLiveLog(logName, forceReload = false) {
  stopPolling();

  const logChanged = state.liveLogName !== logName;
  state.liveLogName = logName;
  if (forceReload || logChanged) {
    state.liveOffset = 0;
  ...
  state.pollTimer = setInterval(async () => {
    try {
      const data = await readChunk(logName, state.liveOffset);
...
(async function init() {
  try {
    ...
    const active = state.logs.find(x => x.is_active) || state.logs.find(x => x.is_live);
    if (active) {
      state.selectedLog = active.name;
      state.liveLogName = active.name;
      els.liveViewerTitle.textContent = `实时日志 (${active.name})`;
      renderLogList();
      els.liveViewer.classList.remove("hidden");
      els.historyViewer.classList.add("hidden");
      startPollingLiveLog(active.name);
    } else {
      setStatus("未找到实时日志", true);
    }
